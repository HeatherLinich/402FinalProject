...
class View extends BuildHTML {
	...
	echo $li_start.'Quick Stats'.$li_end;
	}
	...
	echo '<li id="'.$parent_id.'"><a href="?node='.$parent_link.$parent_desc.'">'.$parent_name.'</a>';
	if (is_array($val)) {
	echo $ul_start;
	...
	//can be abstracted using BuildHTML start element
	echo '<li id="'.$child_id.'"><a href="?node='.$child_link.$child_desc.'">'.$child_name.'</a>'.$li_end;
	}
	}
	echo $ul_end.$li_end;
	}
	...
	function draw_logo() {
		$logo_attributes = array("src"=>DESIGN_IMAGES_DIR."test.png", "alt"=>"framework default logo", "title"=>"402 framework", "class"=>"logo2");
		$header_start = BuildHTML::start_element(self::$div, $logo_attributes);
		$header_end = BuildHTML::end_element(self::$div);
		$logo_start = BuildHTML::start_element(self::$img, $logo_attributes);
		echo $header_start;
		echo '<div id="header_logo" class="grid_7">'.$logo_start.'</div>';
		echo $header_end;
	}
